...
//     }
    ...
    int minFallingPathSum(vector<vector<int>>& matrix) {
        ...
        //tabulation with space optimised (for every possible i, value of j is 0 to c-1 as per the working)
        //base case
        ...
        for(int i = 1; i < r; i++){
            ...
            for(int j = 0; j < c; j++){
                ...
                if(j+1 < c) rd = matrix[i][j] + prev[j+1];
        ...
        for(int i = 0; i < c; i++) ans = min(ans, prev[i]);
